...
public class ClientLoanIntegrationTest {
    ...
    @Test
    public void testLoanScheduleWithInterestRecalculation_WITH_REST_DAILY_INTEREST_COMPOUND_NONE_STRATEGY_REDUCE_EMI_WITH_INSTALLMENT_FEE() {
        this.loanTransactionHelper = new LoanTransactionHelper(this.requestSpec, this.responseSpec);
        ...
        Integer flatInstallmentFee = ChargesHelper.createCharges(requestSpec, responseSpec,
                ChargesHelper.getLoanInstallmentJSON(ChargesHelper.CHARGE_CALCULATION_TYPE_FLAT, "50", false));
        addCharges(charges, flatInstallmentFee, "100", null);
        
        final Integer loanProductID = createLoanProductWithInterestRecalculation(LoanProductTestBuilder.RBI_INDIA_STRATEGY,
                LoanProductTestBuilder.RECALCULATION_COMPOUNDING_METHOD_NONE,
                LoanProductTestBuilder.RECALCULATION_STRATEGY_REDUCE_EMI_AMOUN,
                LoanProductTestBuilder.RECALCULATION_FREQUENCY_TYPE_DAILY, "1", LoanProductTestBuilder.INTEREST_APPLICABLE_STRATEGY_ON_PRE_CLOSE_DATE,
                null, null, null);
        final String graceOnInterestPayment = null;
        final String graceOnPrincipalPayment= null;
        final boolean isDailyInterestCalculation = true;
        final Integer loanID = applyForLoanApplicationForInterestRecalculation(clientID, loanProductID, LOAN_DISBURSEMENT_DATE, LoanApplicationTestBuilder.RBI_INDIA_STRATEGY,
                charges,graceOnInterestPayment,graceOnPrincipalPayment,isDailyInterestCalculation);
        ...
        List<Map<String, Object>> expectedvalues = new ArrayList<>();
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "2482.8", "46.03", "100.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2494.23", "34.6", "100.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2505.71", "23.12", "100.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2517.26", "11.59", "100.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        ...
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "2482.8", "46.03", "100.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2482.8", "46.03", "100.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2505.66", "23.17", "100.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2528.74", "11.64", "100.0", "0.0");
        ...
        expectedvalues = new ArrayList<>();
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -9, true, "2595.95", "32.88", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 2, true, "1862.62", "9.74", "100.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1838.28", "34.08", "100.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1855.32", "17.04", "100.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1847.83", "8.51", "100.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);

        HashMap prepayDetail = this.loanTransactionHelper.getPrepayAmount(this.requestSpec, this.responseSpec, loanID);
        String prepayAmount = String.valueOf(prepayDetail.get("amount"));
        validateNumberForEqualWithMsg("verify pre-close amount", "7647.87", prepayAmount);
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        String LOAN_REPAYMENT_DATE = dateFormat.format(todaysDate.getTime());
        HashMap map = this.loanTransactionHelper.makeRepayment(LOAN_REPAYMENT_DATE, new Float(prepayAmount), loanID);
        loanStatusHashMap = LoanStatusChecker.getStatusOfLoan(this.requestSpec, this.responseSpec, loanID);
        LoanStatusChecker.verifyLoanAccountIsClosed(loanStatusHashMap);
        
        String transactionId =String.valueOf(map.get("resourceId"));
        this.loanTransactionHelper.undoRepayment(loanID, transactionId, LOAN_REPAYMENT_DATE);
        loanStatusHashMap = LoanStatusChecker.getStatusOfLoan(this.requestSpec, this.responseSpec, loanID);
        LoanStatusChecker.verifyLoanIsActive(loanStatusHashMap);
        loanSchedule = this.loanTransactionHelper.getLoanRepaymentSchedule(this.requestSpec, this.responseSpec, loanID);
        expectedvalues = new ArrayList<>();
        ...
        String LOAN_PREPAY_DATE = dateFormat.format(todaysDate.getTime());
        RequestSpecification requestSpecPrePay = new RequestSpecBuilder().setContentType(ContentType.JSON).build();
        requestSpecPrePay.header("Authorization", "Basic " + Utils.loginIntoServerAndGetBase64EncodedAuthenticationKey());
        prepayDetail = this.loanTransactionHelper.getPrepayAmount(requestSpecPrePay, this.responseSpec, loanID,LOAN_PREPAY_DATE);
        prepayAmount = String.valueOf(prepayDetail.get("amount"));
        validateNumberForEqualWithMsg("verify pre-close amount", "7543.0", prepayAmount);
        this.loanTransactionHelper.makeRepayment(LOAN_PREPAY_DATE, new Float(prepayAmount), loanID);
        loanStatusHashMap = LoanStatusChecker.getStatusOfLoan(this.requestSpec, this.responseSpec, loanID);
        LoanStatusChecker.verifyLoanAccountIsClosed(loanStatusHashMap);
    }
    
    @Test
    public void testLoanScheduleWithInterestRecalculation_WITH_REST_DAILY_INTEREST_COMPOUND_NONE_STRATEGY_REDUCE_EMI_PREPAYMENT() {
        this.loanTransactionHelper = new LoanTransactionHelper(this.requestSpec, this.responseSpec);
        ...
        List<Map<String, Object>> expectedvalues = new ArrayList<>();
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "2482.8", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2494.23", "34.6", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2505.71", "23.12", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2517.26", "11.59", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        ...
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "2482.8", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2482.8", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2505.66", "23.17", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2528.74", "11.64", "0.0", "0.0");

        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);

        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        todaysDate.add(Calendar.DAY_OF_MONTH, -9);
        String REPAYMENT_DATE = dateFormat.format(todaysDate.getTime());
       
        this.loanTransactionHelper.makePrepayment(REPAYMENT_DATE, 2000f, loanID);
        loanSchedule = this.loanTransactionHelper.getLoanRepaymentSchedule(this.requestSpec, this.responseSpec, loanID);
        expectedvalues = new ArrayList<>();
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "3979.67", "43.4", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1986.25", "36.82", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2004.5", "18.57", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2029.58", "9.34", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        todaysDate.add(Calendar.DAY_OF_MONTH, -7);
        REPAYMENT_DATE = dateFormat.format(todaysDate.getTime());
        
        ResponseSpecification responseSpec = new ResponseSpecBuilder().expectStatusCode(403).build();
        LoanTransactionHelper validationErrorHelper = new LoanTransactionHelper(this.requestSpec, responseSpec);
        HashMap validationResponse = validationErrorHelper.makePrepayment(REPAYMENT_DATE, 1000f, loanID);
        ArrayList<HashMap> errorData = (ArrayList<HashMap>) validationResponse.get(CommonConstants.RESPONSE_ERROR);
        assertEquals("error.msg.loan.prepayment.cannot.be.made.before.dues.cleared",
                errorData.get(0).get(CommonConstants.RESPONSE_ERROR_MESSAGE_CODE));
        
        
        Float totalDueForCurrentPeriod = (Float) loanSchedule.get(1).get("totalOutstandingForPeriod");
        this.loanTransactionHelper.makeRepayment(REPAYMENT_DATE, totalDueForCurrentPeriod, loanID);
        loanSchedule = this.loanTransactionHelper.getLoanRepaymentSchedule(this.requestSpec, this.responseSpec, loanID);
        expectedvalues = new ArrayList<>();
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "3979.67", "43.4", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1995.36", "27.71", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2004.54", "18.53", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2020.43", "9.3", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        
        this.loanTransactionHelper.makePrepayment(REPAYMENT_DATE, 1000f, loanID);
        loanSchedule = this.loanTransactionHelper.getLoanRepaymentSchedule(this.requestSpec, this.responseSpec, loanID);
        expectedvalues = new ArrayList<>();
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "4979.67", "43.4", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1665.76", "23.11", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1673.43", "15.44", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1681.14", "7.74", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        

        HashMap prepayDetail = this.loanTransactionHelper.getPrepayAmount(this.requestSpec, this.responseSpec, loanID);
        String prepayAmount = String.valueOf(prepayDetail.get("amount"));
        validateNumberForEqualWithMsg("verify pre-close amount", "5043.44", prepayAmount);
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        String LOAN_REPAYMENT_DATE = dateFormat.format(todaysDate.getTime());
        
        validationResponse = validationErrorHelper.makePrepayment(REPAYMENT_DATE, 5030f, loanID);
        errorData = (ArrayList<HashMap>) validationResponse.get(CommonConstants.RESPONSE_ERROR);
        assertEquals("error.msg.loan.prepayment.cannot.be.more.than.principal.due",
                errorData.get(0).get(CommonConstants.RESPONSE_ERROR_MESSAGE_CODE));
    ...
    @Test
    public void testLoanScheduleWithInterestRecalculation_WITH_REST_SAME_AS_REPAYMENT_INTEREST_COMPOUND_NONE_STRATEGY_REDUCE_EMI_PREPAYMENT() {
        this.loanTransactionHelper = new LoanTransactionHelper(this.requestSpec, this.responseSpec);
        ...
        expectedvalues = new ArrayList<>();
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "3977.04", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1986.25", "36.82", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2004.49", "18.58", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2032.22", "9.35", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        ...
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "3977.04", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1995.35", "27.72", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2004.53", "18.54", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2023.08", "9.31", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        ...
        expectedvalues = new ArrayList<>();
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -1, false, "4977.04", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1666.64", "23.12", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1674.31", "15.45", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "1682.01", "7.74", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        

        HashMap prepayDetail = this.loanTransactionHelper.getPrepayAmount(this.requestSpec, this.responseSpec, loanID);
        String prepayAmount = String.valueOf(prepayDetail.get("amount"));
        validateNumberForEqualWithMsg("verify pre-close amount", "5046.08", prepayAmount);
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        String LOAN_REPAYMENT_DATE = dateFormat.format(todaysDate.getTime());
    ...
    @Test
    public void testLoanScheduleWithInterestRecalculation_MATURITY_BEFORE_CURRENT_DATE() {
        this.loanTransactionHelper = new LoanTransactionHelper(this.requestSpec, this.responseSpec);
        ...
        Calendar todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        todaysDate.add(Calendar.DAY_OF_MONTH, -35);
        final String LOAN_DISBURSEMENT_DATE = dateFormat.format(todaysDate.getTime());
        ...
        List<Map<String, Object>> expectedvalues = new ArrayList<>();
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        addRepaymentValues(expectedvalues, todaysDate, -4, false, "2482.8", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2494.23", "34.6", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2505.71", "23.12", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2517.26", "11.59", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        ...
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2482.8", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2482.8", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2551.6", "46.03", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "0.0", "46.03", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);

        
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        todaysDate.add(Calendar.DAY_OF_MONTH, -28);
        String REPAYMENT_DATE = dateFormat.format(todaysDate.getTime());
        Float totalDueForCurrentPeriod = (Float) loanSchedule.get(1).get("totalOutstandingForPeriod");
        this.loanTransactionHelper.makeRepayment(REPAYMENT_DATE, totalDueForCurrentPeriod, loanID);
        loanSchedule = this.loanTransactionHelper.getLoanRepaymentSchedule(this.requestSpec, this.responseSpec, loanID);
        ...
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2494.23", "34.6", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2494.23", "34.6", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "2528.74", "34.6", "0.0", "0.0");
        addRepaymentValues(expectedvalues, todaysDate, 1, false, "0.0", "34.6", "0.0", "0.0");
        verifyLoanRepaymentSchedule(loanSchedule, expectedvalues);
        
        HashMap prepayDetail = this.loanTransactionHelper.getPrepayAmount(this.requestSpec, this.responseSpec, loanID);
        String prepayAmount = String.valueOf(prepayDetail.get("amount"));
        validateNumberForEqualWithMsg("verify pre-close amount", "7655.6", prepayAmount);
        todaysDate = Calendar.getInstance(Utils.getTimeZoneOfTenant());
        String LOAN_REPAYMENT_DATE = dateFormat.format(todaysDate.getTime());
        this.loanTransactionHelper.makeRepayment(LOAN_REPAYMENT_DATE, new Float(prepayAmount), loanID);
        loanStatusHashMap = LoanStatusChecker.getStatusOfLoan(this.requestSpec, this.responseSpec, loanID);
    ...
    private void testLoanScheduleWithInterestRecalculation_WITH_REST_SAME_AS_REPAYMENT_INTEREST_COMPOUND_NONE_STRATEGY_REDUCE_EMI_PRE_CLOSE_INTEREST(
        ...
        final Integer loanProductID = createLoanProductWithInterestRecalculation(LoanProductTestBuilder.DEFAULT_STRATEGY,
                LoanProductTestBuilder.RECALCULATION_COMPOUNDING_METHOD_NONE,
                LoanProductTestBuilder.RECALCULATION_STRATEGY_REDUCE_EMI_AMOUN,
                LoanProductTestBuilder.RECALCULATION_FREQUENCY_TYPE_SAME_AS_REPAYMENT_PERIOD, "0", preCloseInterestStrategy, null, null,
                null);
    ...
    public void testLoanScheduleWithInterestRecalculation_WITH_REST_WEEKLY_INTEREST_COMPOUND_INTEREST_FEE_STRATEGY_REDUCE_NEXT_INSTALLMENTS() {
        ...
        final Integer loanProductID = createLoanProductWithInterestRecalculationAndCompoundingDetails(
                ...
                "1", LoanProductTestBuilder.INTEREST_APPLICABLE_STRATEGY_ON_PRE_CLOSE_DATE, null,
                compoundingDayOfMonth, compoundingDayOfWeek, restDayOfMonth, restDayOfWeek);
    ...
    private void testLoanScheduleWithInterestRecalculation_WITH_REST_WEEKLY_INTEREST_COMPOUND_INTEREST_FEE_STRATEGY_REDUCE_NEXT_INSTALLMENTS_PRE_CLOSE_INTEREST(
        ...
        final Integer loanProductID = createLoanProductWithInterestRecalculationAndCompoundingDetails(
                LoanProductTestBuilder.DEFAULT_STRATEGY, LoanProductTestBuilder.RECALCULATION_COMPOUNDING_METHOD_INTEREST_AND_FEE,
                LoanProductTestBuilder.RECALCULATION_STRATEGY_RESCHEDULE_NEXT_REPAYMENTS,
                LoanProductTestBuilder.RECALCULATION_FREQUENCY_TYPE_WEEKLY, "1", LoanProductTestBuilder.RECALCULATION_FREQUENCY_TYPE_SAME_AS_REPAYMENT_PERIOD,
                null, preCloseInterestStrategy, null, null, null,
                restDateOfMonth, restDateOfWeek);
    ...
    public void testLoanScheduleWithInterestRecalculation_WITH_ORIGINAL_REPAYMENT_BASED_ARREARS_AGEING() {
        ...
        final Integer loanProductID = createLoanProductWithInterestRecalculation(LoanProductTestBuilder.RBI_INDIA_STRATEGY,
                ...
                recalculationCompoundingFrequencyInterval, LoanProductTestBuilder.INTEREST_APPLICABLE_STRATEGY_ON_PRE_CLOSE_DATE,
                null, null, true, null,
                null, getDayOfMonth(todaysDate), getDayOfWeek(todaysDate));
    ...
    private void testLoanScheduleWithInterestRecalculation_FOR_PRE_CLOSE_WITH_MORATORIUM(final String preCloseStrategy,
        ...
        final Integer loanProductID = createLoanProductWithInterestRecalculation(LoanProductTestBuilder.DEFAULT_STRATEGY,
                LoanProductTestBuilder.RECALCULATION_COMPOUNDING_METHOD_NONE,
                LoanProductTestBuilder.RECALCULATION_STRATEGY_REDUCE_EMI_AMOUN,
                LoanProductTestBuilder.RECALCULATION_FREQUENCY_TYPE_SAME_AS_REPAYMENT_PERIOD, "0", preCloseStrategy, null, null, null);
    ...
    private Integer createLoanProductWithInterestRecalculation(final String repaymentStrategy,
            ...
            final String preCloseInterestCalculationStrategy, final Account[] accounts, final Integer recalculationRestFrequencyOnDayType,
    ...
    private Integer createLoanProductWithInterestRecalculation(final String repaymentStrategy,
            ...
            final String recalculationCompoundingFrequencyType, final String recalculationCompoundingFrequencyInterval,
            final String preCloseInterestCalculationStrategy, final Account[] accounts,
            final String chargeId, boolean isArrearsBasedOnOriginalSchedule, final Integer recalculationCompoundingFrequencyOnDayType,
    ...
    private Integer applyForLoanApplicationForInterestRecalculation(final Integer clientID, final Integer loanProductID,
            ...
            final String graceOnInterestPayment, final String graceOnPrincipalPayment) {
        System.out.println("--------------------------------APPLYING FOR LOAN APPLICATION--------------------------------");
        final boolean isDailyInterestCalculation = false;
        return applyForLoanApplicationForInterestRecalculation(clientID, loanProductID, disbursementDate, repaymentStrategy, charges,
                graceOnInterestPayment, graceOnPrincipalPayment, isDailyInterestCalculation);
    }
    
    private Integer applyForLoanApplicationForInterestRecalculation(final Integer clientID, final Integer loanProductID,
            final String disbursementDate, final String repaymentStrategy, final List<HashMap> charges,
            final String graceOnInterestPayment, final String graceOnPrincipalPayment, final boolean isDailyInterestCalculation) {
        System.out.println("--------------------------------APPLYING FOR LOAN APPLICATION--------------------------------");
        LoanApplicationTestBuilder builder = new LoanApplicationTestBuilder() //
                ...
                .withCharges(charges);
        if (isDailyInterestCalculation) {
            builder.withInterestCalculationPeriodTypeAsDays();
        }

        final String loanApplicationJSON = builder.build(clientID.toString(), loanProductID.toString(), null);
        return this.loanTransactionHelper.getLoanId(loanApplicationJSON);
    }
    ...
    public void testLoanInterestRounding() {
        ...
        .withAdjustInterestForRounding(true).withadjustedInstallmentInMultiplesOf(new Integer(1)).build(null));
    ...
    public void testLoanOverdueChargewithCalculationTypePercentageAmountAndFees()
        ...
        final String jobName = SchedulerJobHelper.APPLY_PENALTY_CHARGE_FOR_BROKEN_PERIODS;
        try {
            this.schedulerJobHelper.executeJob(jobName);
        } catch (InterruptedException e) {
            e.printStackTrace();
    ...
    public Integer applyForLoanApplicationWithTranches(final Integer clientID, final Integer loanProductID, String principal,
            List<HashMap> tranches, final String disbursementDate) {
        System.out.println("--------------------------------APPLYING FOR LOAN APPLICATION--------------------------------");
        final String loanApplicationJSON = new LoanApplicationTestBuilder()
                //
                .withPrincipal(principal)
                ...
                .withInterestCalculationPeriodTypeAsDays().withMaxOutstandingLoanBalance("5000000")
