...
public class GroupLoanIndividualMonitoringIntegrationTest {
	...
	public void setup() {
		...
		this.responseSpec = new ResponseSpecBuilder().expectStatusCode(200)
				.build();
		this.responseSpecForStatusCode403 = new ResponseSpecBuilder()
	...
	public void checkGroupLoanCreateAndDisburseFlow() {
		...
		final Integer clientID1 = ClientHelper.createClient(this.requestSpec,
				this.responseSpec);
		final Integer clientID2 = ClientHelper.createClient(this.requestSpec,
				this.responseSpec);
		...
		groupID = GroupHelper.associateClient(this.requestSpec,
				this.responseSpec, groupID.toString(), clientID1.toString());
		groupID = GroupHelper.associateClient(this.requestSpec,
				this.responseSpec, groupID.toString(), clientID2.toString());
		List<HashMap> clientMembers = getClientMemberShare(
				clientID1.toString(), "8000.00", clientID2.toString(),
				"4000.00");
		final Integer loanProductID = createLoanProduct();
		System.out.println("clientMembers :" + clientMembers);
		String loanApplicationDataAsJson = applyForLoanApplication(groupID,
				loanProductID, clientMembers);
		System.out.println("loanApplicationDataAsJson: "+loanApplicationDataAsJson);
		final Integer loanID = this.loanTransactionHelper
				.getLoanId(loanApplicationDataAsJson);
		Assert.assertNotNull(loanID);
		System.out.println("Loan created with id :" + loanID);
		this.groupLoanIndividualMonitoringHelper = new GroupLoanIndividualMonitoringHelper(
				this.requestSpec, this.responseSpec);
		String response = this.groupLoanIndividualMonitoringHelper
				.getClientMembersByLoanId(requestSpec, responseSpec, loanID);
		JsonParser jsonParser = new JsonParser();
		JsonArray jsonArray = (JsonArray) jsonParser.parse(response);
		Float clientsProposedAmount = Float.valueOf("0.0");
		System.out.println("Toatl proposed Amount for Loan : "
				+ (Float.valueOf(proposedPrincipalAmount)));
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			System.out.println("client " + clientName + " with id " + clientId
					+ " have share of: " + clientShare);
			clientsProposedAmount = clientsProposedAmount.sum(
					clientsProposedAmount, clientShare.getAsFloat());
		}
		System.out.println("Total share of clients is :"
				+ clientsProposedAmount);
		Boolean isClientShareEqualToPrincipal = (float) Float
				.valueOf(proposedPrincipalAmount) == (float) Float
				.valueOf(clientsProposedAmount);
	...
	public void invalidProposedClientShare() {
		...
		groupID = GroupHelper.associateClient(this.requestSpec,
				this.responseSpec, groupID.toString(), clientID2.toString());
		List<HashMap> clientMembers = getClientMemberShare(
				clientID1.toString(), "2000.00", clientID2.toString(),
				"3000.00");
		final Integer loanProductID = createLoanProduct();
		this.loanTransactionHelper = new LoanTransactionHelper(
		...
		System.out
				.println("---------- client share not equal to principal amount ----------");
		String loanApplicationJson = applyForLoanApplication(groupID,
				loanProductID, clientMembers);
		List<HashMap<String, Object>> errors = (List<HashMap<String, Object>>) this.loanTransactionHelper
				.createLoanAccount(loanApplicationJson,
		...
		List<HashMap> clientMembers1 = getClientMemberShare(
				clientID1.toString(), "0.00", clientID2.toString(), "12000.00");
		String loanApplicationJson1 = applyForLoanApplication(groupID,
				loanProductID, clientMembers1);
		List<HashMap<String, Object>> errors1 = (List<HashMap<String, Object>>) this.loanTransactionHelper
				.createLoanAccount(loanApplicationJson1,
						CommonConstants.RESPONSE_ERROR);
		assertEquals(
				"error.msg.glim.each.client.must.have.more.than.zero.amount",
	...
	public void approvedClientShare() {
		...
		groupID = GroupHelper.associateClient(this.requestSpec,
				this.responseSpec, groupID.toString(), clientID2.toString());
		List<HashMap> clientMembers = getClientMemberShare(
				clientID1.toString(), "4000.00", clientID2.toString(),
				"8000.00");
		final Integer loanProductID = createLoanProduct();
		String loanApplicationJson = applyForLoanApplication(groupID,
				loanProductID, clientMembers);
		...
		this.groupLoanIndividualMonitoringHelper = new GroupLoanIndividualMonitoringHelper(
				this.requestSpec, this.responseSpec);
		String response = this.groupLoanIndividualMonitoringHelper
				.getClientMembersByLoanId(this.requestSpec, this.responseSpec,
						loanID);
		...
		loanStatusHashMap = this.loanTransactionHelper
				.approveLoanWithApproveAmountAndClientShare(approveDate,
						proposedPrincipalAmount, loanID, clientMembers);
	...
	public void invalidApprovedClientShare() {
		...
		this.loanTransactionHelper = new LoanTransactionHelper(
				this.requestSpec, this.responseSpecForStatusCode403);
		List<HashMap> errors = (List<HashMap>) this.loanTransactionHelper
				.approveLoan(clientMembers, approveDate, approvedAmount,
	...
	public void DisbursedClientShare() {
		...
		this.loanTransactionHelper.disburseLoan(approveDate, loanID,
				proposedPrincipalAmount, clientMembers);
		loanStatusHashMap = LoanStatusChecker.getStatusOfLoan(this.requestSpec,
	...
	public void invalidDisbursementClientShare() {
		...
		this.loanTransactionHelper = new LoanTransactionHelper(
				this.requestSpec, this.responseSpecForStatusCode403);
		this.loanTransactionHelper.disburseLoan(approveDate, loanID,
				"10000.00", clientMembers, CommonConstants.RESPONSE_ERROR);
		List<HashMap> errors = (List<HashMap>) this.loanTransactionHelper
				.disburseLoan(approveDate, loanID, "10000.00", clientMembers,
						CommonConstants.RESPONSE_ERROR);
		assertEquals(
				"error.msg.glim.sum.of.each.clients.share.must.be.equal.to.principal.amount",
				errors.get(0).get(CommonConstants.RESPONSE_ERROR_MESSAGE_CODE));
	...
	private Integer createLoanProduct() {
		System.out
				.println("------------------------------CREATING NEW LOAN PRODUCT ---------------------------------------");
	...
	private String applyForLoanApplication(final Integer groupID,
	...
	private List<HashMap> getClientMemberShare(String clientId1,
			String client1ShareAmount, String clientId2,
