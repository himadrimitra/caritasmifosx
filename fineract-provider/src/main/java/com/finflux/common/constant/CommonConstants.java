package com.finflux.common.constant;

public class CommonConstants {

    // Client related SQL search query parameters
    public static final String CLIENT_DISPLAY_NAME = "clientDisplayName";
    public static final String CLIENT_STATUS = "clientStatus";

    // Center or Group related SQL search query parameters
    public static final String GROUP_DISPLAY_NAME = "groupDisplayName";

    // Loan related SQL search query parameters
    public static final String LOAN_ACCOUNT_NO = "loanAccountNo";
    public static final String LOAN_STATUS = "loan_status";
    public static final String LOAN_EXPECTED_DISBURSEDON_DATE = "loanExpectedDisbursedOnDate";

    // Saving account related SQL search query parameters
    public static final String SAVINGS_ACCOUNT_NO = "savingsAccountNo";
    public static final String SAVINGS_ACCOUNT_STATUS = "savingsAccountStatus";

    // chai_village related SQL search query parameters
    public static final String CHAI_VILLAGE_NAME = "chaiVillageName";

    // Code Value related SQL search query parameters
    public static final String CODE_VALUE_IS_ACTIVE = "codeValueIsActive";

}
