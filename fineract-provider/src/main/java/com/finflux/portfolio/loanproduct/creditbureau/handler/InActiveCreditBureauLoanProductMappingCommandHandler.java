...
@Service
@CommandType(entity = "CREDIT_BUREAU_LOANPRODUCT_MAPPING", action = "INACTIVE")
public class InActiveCreditBureauLoanProductMappingCommandHandler implements NewCommandSourceHandler {

    private final CreditBureauLoanProductMappingWritePlatformService writePlatformService;

    @Autowired
    public InActiveCreditBureauLoanProductMappingCommandHandler(
    ...
    public CommandProcessingResult processCommand(final JsonCommand command) {
        return this.writePlatformService.inActive(command, command.entityId());
    }
}