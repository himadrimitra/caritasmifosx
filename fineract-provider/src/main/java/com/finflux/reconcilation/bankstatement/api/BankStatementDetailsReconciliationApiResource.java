...
public class BankStatementDetailsReconciliationApiResource {
	...
	@PUT
	@Path("{bankStatementDetailId}")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String statementDetailsUpdate(@PathParam("bankStatementId") final Long bankStatementId,
			@PathParam("bankStatementDetailId") final Long bankStatementDetailId, final String apiRequestBodyAsJson,
			@Context final UriInfo uriInfo, @QueryParam("command") String command) {
		final CommandWrapper commandRequest = new CommandWrapperBuilder()
				.updateBankStatementDetails(bankStatementId, bankStatementDetailId).withJson(apiRequestBodyAsJson)
				.build();
		final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
