...
/**
 * 
 * This class is responsible to connect Equifax servers to get Customer credit report. 
 */
@Service
public class EquifaxRequestServiceImpl implements EquifaxRequestService {
	...
	private void populateHeaderProperties(final RequestHeaderType requestHeader,
			final EquifaxCredentialsData equifaxCredentialsData) {
	...
	private RequestBodyType constructRetailRequest(final EnquiryReferenceData enquiryReferenceData,
		...
		requestBody.setInquiryPurpose("0E"); // Hard coding as we need to have mapping between loan products and actual inquiry purpose
		...
		if(loanEnquiryData.getGenderId() != null) {
			if(equifaxCredentialsData.getGenderTypeFemale() != null && loanEnquiryData.getGenderId().equals(equifaxCredentialsData.getGenderTypeFemale())) {
				requestBody.setGender(GenderTypeCode.FEMALE.value()) ;
			}else if(equifaxCredentialsData.getGenderTypeMale() != null && loanEnquiryData.getGenderId().equals(equifaxCredentialsData.getGenderTypeMale())){
		...
		//We are not capturing marital status. Do we need to check relations data?
		...
		if (relations != null && relations.size() > 0) {
			...
			for (EnquiryClientRelationshipData clientRelationData : relations) {
				...
				if (clientRelationData.getRelationshipTypeId() != null) {
					if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeFather())) {
						familyName.setAdditionalNameType(EquifaxConstants.FATHER);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeHusband())) {
						familyName.setAdditionalNameType(EquifaxConstants.HUSBAND);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeBrother())) {
						familyName.setAdditionalNameType(EquifaxConstants.BROTHER);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeSon())) {
						...
						familyName.setAdditionalNameType(EquifaxConstants.SONINLAW);
							...
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeFatherInLaw())) {
						familyName.setAdditionalNameType(EquifaxConstants.FATHERINLAW);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeBrotherInLaw())) {
						familyName.setAdditionalNameType(EquifaxConstants.BROTHERINLAW);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeMother())) {
						familyName.setAdditionalNameType(EquifaxConstants.MOTHER);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeWife())) {
						familyName.setAdditionalNameType(EquifaxConstants.WIFE);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeSister())) {
						...
						familyName.setAdditionalNameType(EquifaxConstants.DAUGHTER);
							...
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeSisterInLaw())) {
						familyName.setAdditionalNameType(EquifaxConstants.SISTERINLAW);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeDaughterInLaw())) {
						familyName.setAdditionalNameType(EquifaxConstants.DAUGHTERINLAW);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeMotherInLaw())) {
						familyName.setAdditionalNameType(EquifaxConstants.MOTHERINLAW);
					} else if (clientRelationData.getRelationshipTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getRelationTypeOther())) {
			...
			for (EnquiryDocumentData clientDocumentData : clientDocumentsList) {
				if (clientDocumentData.getClientIdentificationTypeId() != null) {
					if (clientDocumentData.getClientIdentificationTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getDocumentTypePassport())) {
						requestBody.setPassportId(clientDocumentData.getClientIdentification());
							...
							.equalsIgnoreCase(equifaxCredentialsData.getDocumentTypeVoterId())) {
						requestBody.setVoterId(clientDocumentData.getClientIdentification());
					} else if (clientDocumentData.getClientIdentificationTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getDocumentTypeAadhar())) {
						requestBody.setNationalIdCard(clientDocumentData.getClientIdentification());
					} else if (clientDocumentData.getClientIdentificationTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getDocumentTypeRationCard())) {
						requestBody.setRationCard(clientDocumentData.getClientIdentification());
					} else if (clientDocumentData.getClientIdentificationTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getDocumentTypeDrivingLicense())) {
						requestBody.setDriverLicense(clientDocumentData.getClientIdentification());
					} else if (clientDocumentData.getClientIdentificationTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getDocumentTypePan())) {
						requestBody.setPANId(clientDocumentData.getClientIdentification());
					} else if (clientDocumentData.getClientIdentificationTypeId()
							.equalsIgnoreCase(equifaxCredentialsData.getDocumentTypeOther())) {
	...
	private CreditBureauResponse parseResponse(InquiryRequestType input, InquiryResponseType responseType,
		...
		enquiryResponse = new EnquiryResponse(loanEnquiryReferenceData.getAcknowledgementNumber(), requestString,
				responseString, null, null, CreditBureauEnquiryStatus.SUCCESS,
				loanEnquiryReferenceData.getCbReportId());
