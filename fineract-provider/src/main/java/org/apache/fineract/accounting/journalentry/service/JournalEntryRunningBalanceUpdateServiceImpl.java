...
public class JournalEntryRunningBalanceUpdateServiceImpl implements JournalEntryRunningBalanceUpdateService, BusinessEventListner {
    ...
    public void postConstruct() {
        this.businessEventNotifierService.addBusinessEventPostListners(BUSINESS_EVENTS.GL_ACCOUNT_CLOSURE, this);
    ...
    @Override
    public CommandProcessingResult updateOfficeRunningBalance(final JsonCommand command) {
    ...
    public void businessEventWasExecuted(final Map<BUSINESS_ENTITY, Object> businessEventEntity) {
        final GLClosure glClosure = (GLClosure) businessEventEntity.get(BUSINESS_ENTITY.GL_CLOSURE);
        if (glClosure != null && glClosure.getClosingLocalDate().isEqual(DateUtils.getLocalDateOfTenant())) {
            this.executorService
                    .execute(new updateOfficeRunningBalanceRunnable(ThreadLocalContextUtil.getTenant(), glClosure.getOffice().getId()));
        }
