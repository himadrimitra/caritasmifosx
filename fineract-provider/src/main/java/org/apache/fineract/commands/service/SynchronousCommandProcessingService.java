...
public class SynchronousCommandProcessingService implements CommandProcessingService {
    ...
    private CommandProcessingResult processAndLogCommand(final CommandWrapper wrapper, final JsonCommand command,
        ...
        if (command.commandId() != null) {
            ...
            commandSourceResult.markAsChecked(maker, DateUtils.getLocalDateTimeOfTenant().toDateTime());
        } else {
    ...
    /*
     * processAndLogCommandWithoutTransactionalScope function is only for those
     * tasks which involve multiple object processing and where we should not
     * rollback all the transaction if the failure is on a few objects. For eg.
     * For Periodic Accrual Job we need to process quite a few loans. Just
     * because a few loans accrual failed we should not be roll back the
     * accruals for all the Loans.
     */
    ...
    public boolean validateCommand(final CommandWrapper commandWrapper, final AppUser user) {
        final boolean rollbackTransaction = this.configurationDomainService
                .isMakerCheckerEnabledForTask(commandWrapper.taskPermissionName());
    ...
    private void validateDataAccessPermissions(final CommandWrapper wrapper, final JsonCommand command) {
        final CommandSourceValidator commandSourceValidator = this.commandValidationProvider.getHandler(wrapper.getEntityName());
