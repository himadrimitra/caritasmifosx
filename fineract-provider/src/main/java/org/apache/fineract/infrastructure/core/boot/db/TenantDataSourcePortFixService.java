 ...
 *
 * This service is called by the TenantDatabaseUpgradeService "just at the right
 * time" during start-up, that is AFTER the initialization of the
 * mifosplatform-tenants but BEFORE Flyway runs on all tenant schemas.
 *
...
public class TenantDataSourcePortFixService {
    ...
    public void fixUpTenantsSchemaServerPort() {
        if (!this.enabled) {
            logger.info("No schema_server_port UPDATE made to tenant_server_connections table of the mifosplatform-tenants schema, because "
                    + ENABLED + " = false");
            return;
        }
        if (this.dsp == null) {
            // we don't have any generic mechanism to know the DB port, given
            // just a tenant DataSource
            logger.debug(
                    "No schema_server_port UPDATE made to tenant_server_connections table of the mifosplatform-tenants schema (because neither MariaDB4j nor our own Spring Boot DataSourceConfiguration is used in a traditional WAR)");
            return;
        }
        final int r = this.jdbcTemplate.update(
                "UPDATE tenant_server_connections SET schema_server = ?, schema_server_port = ?, schema_username = ?, schema_password = ?",
                this.dsp.getHost(), this.dsp.getPort(), this.dsp.getUsername(), this.dsp.getPassword());
        if (r == 0) {
            logger.warn("UPDATE tenant_server_connections SET ... did not update ANY rows - something is probably wrong");
        } else {
            logger.info("Upated " + r
                    + " rows in the tenant_server_connections table of the mifosplatform-tenants schema to the real current host: "
                    + this.dsp.getHost() + ", port: " + this.dsp.getPort());
        }
    }
