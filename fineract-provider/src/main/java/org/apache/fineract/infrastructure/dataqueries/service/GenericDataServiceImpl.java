...
public class GenericDataServiceImpl implements GenericDataService {
    ...
    public GenericResultsetData fillGenericResultSet(final String sql) {
        ...
        final List<ResultSetColumnAndData> columnData = null;
    ...
    public List<ResultsetColumnHeaderData> fillResultsetColumnHeaders(final String datatable) {
        ...
        while (columnDefinitions.next()) {
            ...
            while (rsValues.next()) {
                ...
                if (tempColumnName != null && tempColumnName.equalsIgnoreCase(metaDataColumnName)) {
                    ...
                    if (watchColumn > 0 && codeValueId > 0) {
                        ...
                        final String watchColumnName = retreiveWatchColumnName(watchColumn);
                        final Integer codeId = retrieveCodeIdByCodeValueId(codeValueId);
            ...
            final ResultsetColumnHeaderData rsch = ResultsetColumnHeaderData.detailed(columnName, columnType, columnLength, columnNullable,
                    columnIsPrimaryKey, columnValues, codeName, displayName, dependsOnColumnName, orderPosition, visible,
                    mandatoryIfVisible, visibilityCriteria, sectionId);
    ...
    private String retreiveDependsOnColumnName(final Integer dependsOn) {
    ...
    private String retreiveWatchColumnName(final Integer watchColumn) {
    ...
    private SqlRowSet retriveXRegisteredMetadata(final String datatable) {
    ...
    private List<ResultsetColumnValueData> retreiveColumnValues(final String codeName) {
        ...
        final String sql = "select v.id, v.code_score, v.code_value, v.parent_id from m_code m "
                + " join m_code_value v on v.code_id = m.id " + " where m.code_name = '" + codeName + "' order by v.order_position, v.id";
    ...
    @Override
    public List<ScopeCriteriaData> fetchDatatableScopesByIdAndScopingCriteria(final Integer id, final Long scopingCriteriaEnum) {

        final List<ScopeCriteriaData> scopeCriteriaData = new ArrayList<>();
        ...
        final String sql = "select rtf.loan_product_id as loanProductId, rtf.savings_product_id as savingsProductId, rtf.code_value_id as"
                + " codeValueId, rtf.legal_form_enum as legalForm  from f_registered_table_scoping rtf where rtf.registered_table_id = "
                + id + "";
        ...
        if (scopingCriteriaEnum != null && scopingCriteriaEnum > 0) {
            if (DataTableScopes.fromInt(scopingCriteriaEnum.intValue()).isLoanProduct()) {
                final List<AllowedValueOptions> allowedValues = new ArrayList<>();
                ...
                while (rsValues.next()) {
                    ...
                    if (loanProductId != null && loanProductId > 0) {
                        ...
                        final LoanProductData loanProductData = this.loanProductReadPlatformService.retrieveLoanProduct(loanProductId);
                ...
                while (rsValues.next()) {
                    ...
                    if (savingsProductId != null && savingsProductId > 0) {
                        ...
                        final SavingsProductData savingsProductData = this.savingsProductReadPlatformService.retrieveOne(savingsProductId);
                ...
                while (rsValues.next()) {
                    ...
                    if (codeValueId != null && codeValueId > 0) {
                        final CodeValue codeValueData = this.codeValueRepositoryWrapper.findOneWithNotFoundDetection(codeValueId);
                        final Code code = codeValueData.getCode();
                ...
                while (rsValues.next()) {
                    ...
                    if (legalForm != null && legalForm > 0) {
                        ...
                        final EnumOptionData legalFormType = LegalForm.legalFormType(legalForm.intValue());
         ...
         * EnumOptionData data = null; while (rsValues.next()) { final Long
         * loanProductId = rsValues.getLong("loanProductId"); if (loanProductId
         * != null && loanProductId > 0) { data =
         * DataTableScopes.dataTableScopes(DataTableScopes.LOAN_PRODUCT.getId())
         * ; LoanProductData loanProductData =
         * this.loanProductReadPlatformService.retrieveLoanProduct(loanProductId
         * );
         * allowedValues.add(AllowedValueOptions.createNew(loanProductData.getId
         * (), loanProductData.getName())); } }
         * scopeCriteriaData.add(ScopeCriteriaData.createNew(data.getId(),
         * data.getCode(), data.getValue(), allowedValues)); } else
         * if(appTableName != null && appTableName.equals("m_savings_account"))
         * { List<AllowedValueOptions> allowedValues = new ArrayList<>();
         * EnumOptionData data = null; while (rsValues.next()) { final Long
         * savingsProductId = rsValues.getLong("savingsProductId"); if
         * (savingsProductId != null && savingsProductId > 0) { data =
         * DataTableScopes.dataTableScopes(DataTableScopes.SAVINGS_PRODUCT.getId
         * ()); SavingsProductData savingsProductData =
         * this.savingsProductReadPlatformService.retrieveOne(savingsProductId);
         * allowedValues.add(AllowedValueOptions.createNew(savingsProductData.
         * getId(), savingsProductData.getName())); } }
         * scopeCriteriaData.add(ScopeCriteriaData.createNew(data.getId(),
         * data.getCode(), data.getValue(), allowedValues)); } else
         * if(appTableName != null && appTableName.equals("m_client")) {
         * EnumOptionData data = null; while (rsValues.next()) {
         * List<AllowedValueOptions> allowedValues = new ArrayList<>(); final
         * Long codeValueId = rsValues.getLong("codeValueId"); final Long
         * legalForm = rsValues.getLong("legalForm");
         * 
         * if (codeValueId != null && codeValueId > 0) { CodeValue codeValueData
         * = this.codeValueRepositoryWrapper.findOneWithNotFoundDetection(
         * codeValueId); Code code = codeValueData.getCode(); if
         * (code.name().equals("ClientType")) { data =
         * DataTableScopes.dataTableScopes(DataTableScopes.CLIENT_TYPE.getId());
         * allowedValues.add(AllowedValueOptions.createNew(codeValueData.getId()
         * , codeValueData.label()));
         * scopeCriteriaData.add(ScopeCriteriaData.createNew(data.getId(),
         * data.getCode(), data.getValue(), allowedValues)); } else { data =
         * DataTableScopes.dataTableScopes(DataTableScopes.CLIENT_CLASSIFICATION
         * .getId());
         * allowedValues.add(AllowedValueOptions.createNew(codeValueData.getId()
         * , codeValueData.label()));
         * scopeCriteriaData.add(ScopeCriteriaData.createNew(data.getId(),
         * data.getCode(), data.getValue(), allowedValues)); } } else
         * if(legalForm != null && legalForm > 0) { data =
         * DataTableScopes.dataTableScopes(DataTableScopes.CLIENT_LEGAL_FORM.
         * getId()); EnumOptionData legalFormType =
         * LegalForm.legalFormType(legalForm.intValue());
         * allowedValues.add(AllowedValueOptions.createNew(legalFormType.getId()
         * , legalFormType.getValue()));
         * scopeCriteriaData.add(ScopeCriteriaData.createNew(data.getId(),
         * data.getCode(), data.getValue(), allowedValues)); } } }
         */
        return scopeCriteriaData;
    }

    @Override
    public List<SectionData> fetchSections(final Integer dataTableId) {
        ...
        while (rsValues.next()) {
            final Long id = rsValues.getLong("id");
            final String displayName = rsValues.getString("display_name");
            final Integer displayPosition = rsValues.getInt("display_position");
            final SectionData sectionData = SectionData.instance(id, displayName, displayPosition);
    ...
    private Integer retrieveCodeIdByCodeValueId(final Integer codeValueId) {
