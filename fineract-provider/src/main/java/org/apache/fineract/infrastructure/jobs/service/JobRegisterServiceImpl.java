...
public class JobRegisterServiceImpl implements JobRegisterService, ApplicationListener<ContextClosedEvent> {
    ...
    @Autowired
    public void setSchedularWritePlatformService(final SchedularWritePlatformService schedularWritePlatformService) {
        this.schedularWritePlatformService = schedularWritePlatformService;
    }

    @Autowired
    public void setTenantDetailsService(final TenantDetailsService tenantDetailsService) {
        this.tenantDetailsService = tenantDetailsService;
    }

    @Autowired
    public void setSchedulerJobListener(final SchedulerJobListener schedulerJobListener) {
        this.schedulerJobListener = schedulerJobListener;
    }

    @Autowired
    public void setSchedulerStopListener(final SchedulerStopListener schedulerStopListener) {
        this.schedulerStopListener = schedulerStopListener;
    }

    @Autowired
    public void setGlobalTriggerListener(final SchedulerTriggerListener globalTriggerListener) {
    ...
    public void loadAllJobs() {
        ...
        for (final FineractPlatformTenant tenant : allTenants) {
            ...
            final boolean loadJobs = isJobLoadingRequired();
            if (loadJobs) {
    ...
    private boolean isJobLoadingRequired() {
        ...
        if (environment.containsProperty("loadJobsForTenantIdentifier")) {
            final String tenamtIdentifier = ThreadLocalContextUtil.getTenant().getTenantIdentifier();
            loadJobs = false;
            final String tenantsAsString = environment.getProperty("loadJobsForTenantIdentifier");
            if (tenantsAsString != null) {
                final String[] tenants = tenantsAsString.split(":");
    ...
    public void executeJob(final ScheduledJobDetail scheduledJobDetail, String triggerType, final Map<String, Object> jobParams) {
        try {
            ...
            if (scheduler == null || !scheduler.checkExists(jobKey)) {
                ...
                final Scheduler tempScheduler = createScheduler(tempSchedulerName, 1, this.schedulerJobListener,
                        this.schedulerStopListener);
                ...
                for (final ScheduledJobDetail jobDetail : scheduledJobDetails) {
                    if (jobDetail.isTriggerMisfired()) {
                        if (jobDetail.isActiveSchedular()) {
                            final Map<String, Object> jobParams = new HashMap<>(1);
     ...
     */
    @Override
    public void onApplicationEvent(@SuppressWarnings("unused") final ContextClosedEvent event) {
        stopAllSchedulers();
    }
    ...
    private Scheduler getScheduler(final ScheduledJobDetail scheduledJobDetail) throws Exception {
        ...
        if (scheduler == null) {
            ...
            scheduler = createScheduler(schedulername, noOfThreads, this.schedulerJobListener);
    ...
    private Scheduler createScheduler(final String name, final int noOfThreads, final JobListener... jobListeners) throws Exception {
        final SchedulerFactoryBean schedulerFactoryBean = new SchedulerFactoryBean();
        schedulerFactoryBean.setSchedulerName(name);
        schedulerFactoryBean.setGlobalJobListeners(jobListeners);
        final TriggerListener[] globalTriggerListeners = { this.globalSchedulerTriggerListener };
