...
public final class SmsDataValidator {
    ...
    public void validateQueryParams(final Integer status, final DateParam dateFrom,
			final DateParam dateTo, final String dateFormat){
    	if( status != null){
    		if (SmsMessageStatusType.fromInt(status) == SmsMessageStatusType.INVALID){
    			throw new InvalidSmsStatusException(status.toString());
    		}
    	}
    	
    	if(dateFrom != null || dateTo != null){
    		if( dateFormat == null){
				throw new GeneralPlatformDomainRuleException("error.msg.sms.dateformat.cannot.be.empty",
