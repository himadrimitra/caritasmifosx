...
public class SmsReadPlatformServiceImpl implements SmsReadPlatformService {
    ...
    public Page<SmsData> retrieveAll(final SearchParameters searchParameters) {
        ...
        if (dateFrom != null && dateTo != null) { return this.paginationHelper.fetchPage(this.jdbcTemplate, sqlCountRows,
                sqlBuilder.toString(), new Object[] { fromDate, toDate }, this.smsRowMapper); }
    ...
    public Collection<SmsData> retrieveAllPending(final Integer limit) {
        ...
        final String sql = "select " + this.smsRowMapper.schema() + " where smo.status_enum = " + SmsMessageStatusType.PENDING.getValue()
                + sqlPlusLimit;
