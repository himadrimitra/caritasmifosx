...
public class FundsApiResource {
    ...
    @Produces({ MediaType.APPLICATION_JSON })
    public String assignFund(@PathParam("fundId") final Long fundId,FormDataMultiPart formParams) throws InvalidFormatException, IOException {
    	
    	this.context.authenticatedUser();
    	final CommandWrapper commandRequest = new CommandWrapperBuilder().assignFund(fundId,"fromcsv").withFormDataMultiPart(formParams).build();
    ...
    //this is to get matching loans based on search criteria, actually  its GET method
    @POST
    @Path("/mapping/loans")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String fundAdvancedSearch(@Context final UriInfo uriInfo, final String json, @QueryParam("isSummary") final Boolean isSummary) {
        ...
        if (isSummary) {
            ...
            boolean isDetailedQuery = true;
            fundSearchQueryBuilder = this.fundMappingQueryBuilderService.getQuery(json,isDetailedQuery);
        }
        List<FundMappingSearchData> fundMappingSearchData = this.fundMappingQueryBuilderService.getSearchedData(fundSearchQueryBuilder);
