...
public class FundReadPlatformServiceImpl implements FundReadPlatformService {
    ...
    public static final class FundMapper implements RowMapper<FundData> {
        ...
        public String schema() {
            ...
            sql.append(
                    "fundRepaymentFrequencyCodeValue.id as fundRepaymentFrequencyId, fundRepaymentFrequencyCodeValue.code_value as fundRepaymentFrequencyValue, ");
            ...
            sql.append(
                    " LEFT JOIN  m_code_value fundRepaymentFrequencyCodeValue on fundRepaymentFrequencyCodeValue.id = fund.fund_repayment_frequency ");
            return sql.toString();
        ...
        public FundData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
            ...
            final CodeValueData fundSource = CodeValueData.instance(fundSourceId, fundSourceValue);

            final Long fundCategoryId = rs.getLong("fundCategoryId");
            final String fundCategoryValue = rs.getString("fundCategoryValue");
            final CodeValueData fundCategory = CodeValueData.instance(fundCategoryId, fundCategoryValue);

            final Long facilityTypeId = rs.getLong("facilityTypeId");
            final String facilityTypeValue = rs.getString("facilityTypeValue");
            final CodeValueData facilityType = CodeValueData.instance(facilityTypeId, facilityTypeValue);
            ...
            final CodeValueData fundRepaymentFrequency = CodeValueData.instance(fundRepaymentFrequencyId, fundRepaymentFrequencyValue);
            ...
            final Boolean isLoanAssigned = rs.getBoolean("isLoanAssigned");
    ...
    public Collection<FundData> retrieveAllFunds(final String command) {
        ...
        final Collection<FundData> fundDataList = this.jdbcTemplate.query(sql, rm, new Object[] {});
        for (final FundData fundData : fundDataList) {
    ...
    public Map<String, Object> retrieveTemplate(final String command) {
        ...
        if (command != null && command.equalsIgnoreCase("search")) {
            ...
            final List<FundData> fundOptions = (List<FundData>) retrieveAllFunds(FundApiConstants.activeParamName);

            final List<EnumOptionData> conditionalOperatorOptions = this.dropdownReadPlatformService.retrieveConditionalOperatorOptions();
    ...
    public static final class FundLoanPurposeMapper implements RowMapper<FundLoanPurposeData> {
        ...
        public String schema() {
            return " flp.id as id ,flp.loan_purpose_amount loanPurposeAmount, f.id as fundId, lp.id as loanPurposeId, "
                    + "lp.code_value as loanPurposeCodeValue, f.sanctioned_amount as sanctionedAmount " + " from f_fund_loan_purpose flp "
                    + "left join m_fund f on f.id = flp.fund_id " + "left join m_code_value lp on lp.id = flp.loan_purpose_id ";
        }

        @SuppressWarnings("unused")
        @Override
        public FundLoanPurposeData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
            final Long id = JdbcSupport.getLong(rs, "id");
            ...
            final CodeValueData loanPurpose = CodeValueData.instance(loanPurposeId, loanPurposeCodeValue);
    ...
    @Override
    public List<FundLoanPurposeData> retrieveLoanPurposeByFundId(final Long fundId) {
        final FundLoanPurposeMapper fundLoanPurposeMapper = new FundLoanPurposeMapper();
        final String sql = "select " + fundLoanPurposeMapper.schema() + " where flp.fund_id = ? ";
