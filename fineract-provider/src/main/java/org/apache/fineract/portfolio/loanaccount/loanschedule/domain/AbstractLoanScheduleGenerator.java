...
public abstract class AbstractLoanScheduleGenerator implements LoanScheduleGenerator {
        ...
        if (!scheduleParams.isPartialUpdate()) {
            ...
            if (loanApplicationTerms.isMultiDisburseLoan()) {
                ...
                if(loanApplicationTerms.isEmiBasedOnDisbursements()) {
                	adjustInstallmentOrPrincipalAmount(loanApplicationTerms, Money.zero(loanApplicationTerms.getCurrency()), scheduleParams.getPeriodNumber(), mc);	
                }
            }
        }
    ...
    private void applyChargesForCurrentPeriod(final Set<LoanCharge> loanCharges, final MonetaryCurrency currency,
            ...
            final LoanApplicationTerms loanApplicationTerm) {
        ...
        currentPeriodParams.setFeeChargesForInstallment(cumulativeFeeChargesDueWithin(scheduleParams.getPeriodStartDate(),
                ...
                scheduleParams.getTotalCumulativeInterest(), true, loanApplicationTerm, scheduleParams.getInstalmentNumber()));
        ...
        for (LoanScheduleModelPeriod loanScheduleModelPeriod : periods) {
            if (loanScheduleModelPeriod.isRepaymentPeriod()) {
                ...
                Money feeChargesForInstallment = cumulativeFeeChargesDueWithin(loanScheduleModelPeriod.periodFromDate(),
                        ...
                        !loanScheduleModelPeriod.isRecalculatedInterestComponent(), loanApplicationTerms, loanScheduleModelPeriod.periodNumber());
    ...
    private LoanScheduleModelPeriod handlePrepaymentOfLoan(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        if (!scheduleParams.getOutstandingBalance().isGreaterThan(currentPeriodParams.getInterestForThisPeriod())
            ...
            applyChargesForCurrentPeriod(loanCharges, totalInterestChargedForFullLoanTerm.getCurrency(), scheduleParams, calculateTill,
                    tempPeriod, loanApplicationTerms);
                    ...
                    if (isPrepay) {
                        ...
                        if (!transactionDate.isEqual(scheduleParams.getPeriodStartDate()) || scheduleParams.getInstalmentNumber() == 1) {
                            ...
                            Money feeChargesForInstallment = cumulativeFeeChargesDueWithin(scheduleParams.getPeriodStartDate(),
                                    ...
                                    scheduleParams.getTotalCumulativeInterest(), false, loanApplicationTerms, scheduleParams.getInstalmentNumber());
    ...
    private void updateBalanceBasedOnDisbursement(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        if(scheduleParams.isResetEMI()){
            adjustInstallmentOrPrincipalAmount(loanApplicationTerms, scheduleParams.getTotalCumulativePrincipal(), scheduleParams.getPeriodNumber(), mc);       
    ...
    private LoanTermVariationParams applyExceptionLoanTermVariations(final LoanApplicationTerms loanApplicationTerms,
            ...
            final Money totalCumulativePrincipal, final Money totalCumulativeInterest, final MathContext mc) {
    boolean skipPeriod = false;
        boolean recalculateAmounts = false;
        LocalDate modifiedScheduledDueDate = scheduledDueDate;
    ...
    private Money getPrincipalToBeScheduled(final LoanApplicationTerms loanApplicationTerms) {
        ...
        if (loanApplicationTerms.isMultiDisburseLoan() && loanApplicationTerms.getApprovedPrincipal().isGreaterThanZero() && !loanApplicationTerms.isEmiBasedOnDisbursements()) {
            ...
            while (compoundingDate.isBefore(endDate)) {
                ...
                if (compoundingDate.isBefore(endDate)) {
                    Money feeChargesForInstallment = cumulativeFeeChargesDueWithin(lastCompoundingDate, compoundingDate, charges, currency,
                            null, loanApplicationTerms.getPrincipal(), null, false, loanApplicationTerms, scheduleParams.getInstalmentNumber());
                    Money penaltyChargesForInstallment = cumulativePenaltyChargesDueWithin(lastCompoundingDate, compoundingDate, charges,
                            currency, null, loanApplicationTerms.getPrincipal(), null, false, loanApplicationTerms, scheduleParams.getInstalmentNumber());
    ...
    private Money cumulativeFeeChargesDueWithin(final LocalDate periodStart, final LocalDate periodEnd, final Set<LoanCharge> loanCharges,
            ...
            final LoanApplicationTerms loanApplicationTerm, int installmentNumber) {
            ...
            if (!loanCharge.isDueAtDisbursement() && loanCharge.isFeeCharge()) {
                if (loanCharge.isInstalmentFee() && isInstallmentChargeApplicable) {                    
                    if(loanCharge.getChargeCalculation().isPercentageOfDisbursementAmount()){
                    	cumulative = cumulative.plus(calculateInstallmentCharge(principalInterestForThisPeriod, cumulative, loanCharge, loanApplicationTerm, installmentNumber));
                    }else{
                    	cumulative = calculateInstallmentCharge(principalInterestForThisPeriod, cumulative, loanCharge, loanApplicationTerm, installmentNumber);
                    }
    ...
    private Money calculateInstallmentCharge(final PrincipalInterest principalInterestForThisPeriod, Money cumulative,
            final LoanCharge loanCharge, final LoanApplicationTerms loanApplicationTerm, int periodNumber) {
        if (loanCharge.getChargeCalculation().isPercentageBased()) {
            BigDecimal amount = BigDecimal.ZERO;
            if (loanCharge.getChargeCalculation().isPercentageOfAmountAndInterest()) {
    ...
    private LoanScheduleDTO rescheduleNextInstallments(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        if (rescheduleFrom != null) {
            ...
            if (loanApplicationTerms.isMultiDisburseLoan()) {
                ...
                if(loanApplicationTerms.isEmiBasedOnDisbursements()) {
                	adjustInstallmentOrPrincipalAmount(loanApplicationTerms, Money.zero(loanApplicationTerms.getCurrency()), periodNumber, mc);	
                }
            }
    ...
    public BigDecimal calculateFirstInstallmentAmount(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        if (loanApplicationTerms.adjustFirstEMIAmount()) {
            ...
            final LocalDate idealDisbursementDate = this.scheduledDateGenerator.idealDisbursementDateBasedOnFirstRepaymentDate(
                    ...
                    loanApplicationTerms.getLoanCalendar(), loanApplicationTerms.getHolidayDetailDTO(), loanApplicationTerms);
            LocalDate periodStartDateApplicableForInterest = calculateInterestStartDateForPeriod(loanApplicationTerms,
                    loanApplicationTerms.getExpectedDisbursementDate(), idealDisbursementDate, firstRepaymentdate,
                    loanApplicationTerms.isInterestChargedFromDateSameAsDisbursalDateEnabled(),
                    loanApplicationTerms.getExpectedDisbursementDate());
