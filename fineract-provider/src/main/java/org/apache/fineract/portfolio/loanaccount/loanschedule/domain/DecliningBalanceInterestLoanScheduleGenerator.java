...
public class DecliningBalanceInterestLoanScheduleGenerator extends AbstractLoanScheduleGenerator {
    ...
    public PrincipalInterest calculatePrincipalInterestComponentsForPeriod(final PaymentPeriodsInOneYearCalculator calculator,
            ...
            final Money totalCumulativeInterest, @SuppressWarnings("unused") final Money totalInterestDueForLoan,
        ...
        final Map<LocalDate, BigDecimal> interestRates = new HashMap<>(termVariations.size());

        for (final LoanTermVariationsData loanTermVariation : termVariations) {
        ...
        if (principalVariation != null) {
            ...
            for (final Map.Entry<LocalDate, Money> principal : principalVariation.entrySet()) {

                if (!principal.getKey().isAfter(periodEndDate)) {
                    final int interestForDays = Days.daysBetween(interestStartDate, principal.getKey()).getDays();
        ...
        interestForThisInstallment = interestForThisInstallment.plus(result.interest());
        cumulatingInterestDueToGrace = result.interestPaymentDueToGrace();
        interestForThisInstallment = loanApplicationTerms.adjustInterestBasedOnTotalInterestForSchedule(interestForThisInstallment,
        ...
        } else if (cumulatingInterestDueToGrace.isGreaterThanZero()) {
            interestForPeriod = cumulatingInterestDueToGrace;
            if (cumulatingInterestPaymentDueToGrace.isGreaterThanZero()) {
                interestForPeriod = interestForPeriod.minus(cumulatingInterestPaymentDueToGrace);
            } else {
                interestForPeriod = interestForPeriod.plus(cumulatingInterestPaymentDueToGrace);
