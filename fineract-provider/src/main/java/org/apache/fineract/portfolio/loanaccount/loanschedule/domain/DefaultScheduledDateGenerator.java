...
public class DefaultScheduledDateGenerator implements ScheduledDateGenerator {
    ...
    private boolean useCalendar(final LocalDate lastRepaymentDate, final LoanApplicationTerms loanApplicationTerms,
        ...
        final LocalDate calendarCreationDate = DateUtils.getLocalDateOfTenant().withDayOfMonth(1).withYear(2017).withMonthOfYear(8);
        final Date calendarStartDate = currentCalendar.getCreatedDate() == null ? DateUtils.getLocalDateOfTenant().toDate()
                : currentCalendar.getCreatedDate().toDate();
        if (calendarStartDate.after(calendarCreationDate.toDate())) { return true; }
        final LocalDate newCodeMigrationDate = DateUtils.getLocalDateOfTenant().withDayOfMonth(8).withYear(2016).withMonthOfYear(4);
    ...
    private LocalDate adjustToNthWeekDay(LocalDate dueRepaymentPeriodDate, final int nthDay, final int dayOfWeek) {
    ...
    public Boolean isDateFallsInSchedule(final PeriodFrequencyType frequency, final int repaidEvery, final LocalDate startDate,
        ...
        switch (frequency) {
            case DAYS:
                final int diff = Days.daysBetween(startDate, date).getDays();
                isScheduledDate = (diff % repaidEvery) == 0;
            break;
            case WEEKS:
                final int weekDiff = Weeks.weeksBetween(startDate, date).getWeeks();
                isScheduledDate = (weekDiff % repaidEvery) == 0;
                if (isScheduledDate) {
                    final LocalDate modifiedDate = startDate.plusWeeks(weekDiff);
                    isScheduledDate = modifiedDate.isEqual(date);
                }
            break;
            case MONTHS:
                final int monthDiff = Months.monthsBetween(startDate, date).getMonths();
                isScheduledDate = (monthDiff % repaidEvery) == 0;
                if (isScheduledDate) {
                    final LocalDate modifiedDate = startDate.plusMonths(monthDiff);
                    isScheduledDate = modifiedDate.isEqual(date);
                }
            break;
            case YEARS:
                final int yearDiff = Years.yearsBetween(startDate, date).getYears();
                isScheduledDate = (yearDiff % repaidEvery) == 0;
                if (isScheduledDate) {
                    final LocalDate modifiedDate = startDate.plusYears(yearDiff);
                    isScheduledDate = modifiedDate.isEqual(date);
                }
            break;
            case INVALID:
            ...
            case MONTHS:
                if (loanCalendar == null) {
                    ...
                    while (idealDisbursementDateTemp.isBefore(firstRepaymentDate)) {
                        ...
                        idealDisbursementDateTemp = CalendarUtils.getNewRepaymentMeetingDate(recurrence, meetingStartDate,
                                idealDisbursementDateTemp.plusDays(1), repaidEvery,
    ...
    /**
     * It will validate working days exemption and generate the repayment date
     * based on the config
     * 
     * @param adjustedDateDetailsDTO
     * @param holidayDetailDTO
        ...
        if (seedDate != null) {
            ...
            for (final WorkingDayExemptionsData workingDayExcemption : workingDayExemptions) {
                if (ExpressionUtilsServiceImpl.isGivenDateisWorkingDayExemption(adjustedDateDetailsDTO.getChangedScheduleDate(),
                    ...
                    } while (ExpressionUtilsServiceImpl.isGivenDateisWorkingDayExemption(scheduleDate,
                            workingDayExcemption.getExpression()));
                    if (workingDayExcemption.getRepaymentScheduleUpdationType().isExtendRepaymentSchedule()) {
                        ...
                        /**
                         * Due to Extend Repayment Schedule setting we are
                         * changing the Actual Repayment Date to generate next
                         * repayments date should extend accordingly
                         */
    ...
    @Override
    public LocalDate calculateInterestStartDateForPeriod(final LoanApplicationTerms loanApplicationTerms, final LocalDate periodStartDate,
