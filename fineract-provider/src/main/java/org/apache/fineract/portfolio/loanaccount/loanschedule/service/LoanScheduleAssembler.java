...
public class LoanScheduleAssembler {
    ...
    public LoanApplicationTerms assembleLoanTerms(final JsonElement element, final boolean considerAllDisbursmentsInSchedule) {
        ...
        final LoanApplicationTerms terms = assembleLoanTerms(element, considerAllDisbursmentsInSchedule, loanProduct);
    ...
    public LoanApplicationTerms assembleLoanTerms(final JsonElement element, final boolean considerAllDisbursmentsInSchedule,
        ...
        final LoanApplicationTerms terms = assembleLoanApplicationTermsFrom(element, loanProduct, considerAllDisbursmentsInSchedule);
        if (loanProduct.isFlatInterestRate()) {
            ...
            final double annualIrr = IRRCalculator.calculateIrr(terms);
            if (Double.isNaN(annualIrr)) { throw new IRRCalculationException(); }
            final RoundingMode roundingMode = MoneyHelper.getRoundingMode();
            final MathContext mc = new MathContext(8, roundingMode);
            final Money emi = terms.calculateEmiWithFlatInterestRate(mc);
            ...
            if (terms.getInterestRatePeriodFrequencyType().isMonthly()) {
                final double monthlyRate = annualIrr / 12;
            ...
            if (terms.isAdjustInterestForRounding()) {
                ...
                final Money principal = terms.getPrincipalToBeScheduled();
    ...
    private LoanApplicationTerms assembleLoanApplicationTermsFrom(final JsonElement element, final LoanProduct loanProduct,
        ...
        if (nthDayType.isOnDay()) {
            final String[] repeatsOnDayOfMonthString = this.fromApiJsonHelper
                    .extractArrayNamed(CALENDAR_SUPPORTED_PARAMETERS.REPEATS_ON_DAY_OF_MONTH.getValue(), element);
        ...
        final boolean isEmiBasedOnDisbursements = loanProduct.getLoanProductRelatedDetail().isEmiBasedOnDisbursements();
        final InterestCalculationPeriodMethod pmtCalculationPeriodMethod = loanProduct.getLoanProductRelatedDetail()
                .getPmtCalculationPeriodMethod();
        ...
        final Integer brokenPeriodType = this.fromApiJsonHelper
                .extractIntegerWithLocaleNamed(LoanProductConstants.brokenPeriodMethodTypeParamName, element);
        ...
        BigDecimal discountOnDisburseAmount = this.fromApiJsonHelper
        ...
        if ((loanType.isJLGAccount() || loanType.isGroupAccount()) && calendarId != null) {
            ...
            this.loanScheduleValidator.validateRepaymentFrequencyIsSameAsMeetingFrequency(meetingPeriodFrequency.getValue(),
        ...
        final Integer minimumDaysBetweenDisbursalAndFirstRepayment = this.loanUtilService
                .calculateMinimumDaysBetweenDisbursalAndFirstRepayment(expectedDisbursementDate, loanProduct, loanTermPeriodFrequencyType,
                        repaymentEvery);

        if (calculatedRepaymentsStartingFromDate == null) {

            calculatedRepaymentsStartingFromDate = this.loanUtilService.getCalculatedRepaymentsStartingFromDate(repaymentEvery,
                    expectedDisbursementDate, repaymentPeriodFrequencyType, minimumDaysBetweenDisbursalAndFirstRepayment, calendar, group);
        }
        ...
        if (!disbursementDatas.isEmpty()) {
            ...
            for (final DisbursementData data : disbursementDatas) {
        ...
        if (isInterestRecalculationEnabled) {
            final LoanProductInterestRecalculationDetails loanProductInterestRecalculationDetails = loanProduct
            ...
            final Integer frequency = loanProductInterestRecalculationDetails.getRestInterval();
            if (recalculationFrequencyType.isSameAsRepayment()) {
                ...
                final LocalDate restStartDate = this.fromApiJsonHelper
            ...
            if (compoundingMethod.isCompoundingEnabled()) {
                ...
                if (compoundingFrequencyType.isSameAsRepayment()) {
                    ...
                    LocalDate calendarStartDate = expectedDisbursementDate;
                    final LocalDate compoundingStartDate = this.fromApiJsonHelper
        ...
        if (loanProduct.isLinkedToFloatingInterestRate()) {
            ...
            final FloatingRateDTO floatingRateDTO = new FloatingRateDTO(isFloatingInterestRate, expectedDisbursementDate, interestRateDiff,
                    baseLendingRatePeriods);
            final Collection<FloatingRatePeriodData> applicableRates = loanProduct.fetchInterestRates(floatingRateDTO);

            LocalDate interestRateStartDate = DateUtils.getLocalDateOfTenant();
            final LocalDate dateValue = null;
            final boolean isSpecificToInstallment = false;
            for (final FloatingRatePeriodData periodData : applicableRates) {
                final LoanTermVariationsData loanTermVariation = new LoanTermVariationsData(
        ...
        /*
         * if their is a variations in EMI amount For Multi disbursal Loan get
         * the EMI amounts from LoanTermVariation table
         */
        final Long loanId = this.fromApiJsonHelper.extractLongNamed("id", element);
        if (loanId != null) {
            final Loan loan = this.loanRepositoryWrapper.findOneWithNotFoundDetection(loanId);
            final List<LoanTermVariations> loanEmiTermVariations = loan.getLoanTermVariations();
            for (final LoanTermVariations loanEmiTermVariation : loanEmiTermVariations) {
                final LoanTermVariationsData loanTermVariation = new LoanTermVariationsData(
                        LoanEnumerations.loanvariationType(LoanTermVariationType.EMI_AMOUNT),
                        new LocalDate(loanEmiTermVariation.getTermApplicableFrom()), loanEmiTermVariation.getTermValue(), null, false);
                loanTermVariations.add(loanTermVariation);
            }
        }
        ...
        final HolidayDetailDTO detailDTO = new HolidayDetailDTO(isHolidayEnabled, holidays, workingDays, workingDayExemptions);
        ...
        final LoanApplicationTerms loanApplicationTerms = LoanApplicationTerms.assembleFrom(applicationCurrency, loanTermFrequency,
                ...
                calendar, calendarHistoryDataWrapper, BigDecimal.ZERO, loanTermVariations,
                isInterestChargedFromDateSameAsDisbursalDateEnabled, numberOfDays, isSkipMeetingOnFirstDay, detailDTO,
                allowCompoundingOnEod, isSubsidyApplicable, firstEmiAmount, loanProduct.getAdjustedInstallmentInMultiplesOf(),
                loanProduct.adjustFirstEMIAmount(), considerFutureDisbursmentsInSchedule, considerAllDisbursmentsInSchedule,
                loanProduct.getWeeksInYearType(), loanProduct.isAdjustInterestForRounding(), isEmiBasedOnDisbursements,
                pmtCalculationPeriodMethod, brokenPeriodMethod, loanProduct.allowNegativeLoanBalances(), amountForUpfrontCollection,
                discountOnDisburseAmount);
    ...
    private CalendarInstance createCalendarForSameAsRepayment(final Integer repaymentEvery,
        ...
        final CalendarInstance restCalendarInstance = createInterestRecalculationCalendarInstance(expectedDisbursementDate, repaymentEvery,
    ...
    private CalendarInstance createInterestRecalculationCalendarInstance(final LocalDate calendarStartDate, final Integer frequency,
            final CalendarFrequencyType calendarFrequencyType, final Integer recalculationFrequencyNthDay, final Integer repeatsOnDay) {
    ...
    private Calendar createLoanCalendar(final LocalDate calendarStartDate, final Integer frequency,
            final CalendarFrequencyType calendarFrequencyType, final Integer repeatsOnDay, final Integer repeatsOnNthDayOfMonth,
    ...
    private List<DisbursementData> fetchDisbursementData(final JsonObject command) {
        ...
        final List<DisbursementData> disbursementDatas = new ArrayList<>();
            ...
            if (disbursementDataArray != null && disbursementDataArray.size() > 0) {
                ...
                do {
                    ...
                    if (jsonObject.has(LoanApiConstants.discountOnDisbursalAmountParameterName)
                            && jsonObject.get(LoanApiConstants.discountOnDisbursalAmountParameterName).isJsonPrimitive() && StringUtils
                                    .isNotBlank((jsonObject.get(LoanApiConstants.discountOnDisbursalAmountParameterName).getAsString()))) {
                        discountOnDisbursalAmount = jsonObject.getAsJsonPrimitive(LoanApiConstants.discountOnDisbursalAmountParameterName)
                                .getAsBigDecimal();
                    }

                    disbursementDatas.add(
                            new DisbursementData(null, expectedDisbursementDate, null, principal, null, null, discountOnDisbursalAmount));
                    i++;
    ...
    public LoanScheduleModel assembleLoanScheduleFrom(final JsonElement element, final LoanApplicationTerms loanApplicationTerms) {
        ...
        final List<GroupLoanIndividualMonitoring> glimList = this.groupLoanIndividualMonitoringAssembler
                .createOrUpdateIndividualClientsAmountSplit(null, element, interestRate, numberOfRepayments,
        ...
        final List<Holiday> holidays = this.holidayRepository.findByOfficeIdAndGreaterThanDate(officeId, expectedDisbursementDate.toDate(),
                HolidayStatusType.ACTIVE.getValue());
        final WorkingDays workingDays = this.workingDaysRepository.findOne();
        this.loanScheduleValidator.validateDisbursementDateIsOnHolidayOrNonWorkingDay(loanApplicationTerms.getExpectedDisbursementDate(),
                workingDays, isHolidayEnabled, holidays);
        final List<LoanDisbursementDetails> loanDisbursementDetails = this.loanUtilService.fetchDisbursementData(element.getAsJsonObject());
    ...
    public LoanScheduleModel assembleLoanScheduleFrom(final LoanApplicationTerms loanApplicationTerms, final boolean isHolidayEnabled,
            ...
            final List<LoanDisbursementDetails> disbursementDetails, final List<GroupLoanIndividualMonitoring> glimList) {
        ...
        if (glimList != null && !glimList.isEmpty()) {
            this.groupLoanIndividualMonitoringAssembler.adjustRoundOffValuesToApplicableCharges(loanCharges,
                    loanApplicationTerms.getNumberOfRepayments(), glimList);
        ...
        final BigDecimal firstFixedEmiAmount = GroupLoanIndividualMonitoringAssembler
        ...
        if (loanApplicationTerms.adjustFirstEMIAmount()) {
            final BigDecimal firstEmiAmount = loanScheduleGenerator.calculateFirstInstallmentAmount(mc, loanApplicationTerms, loanCharges,
                    detailDTO);
        ...
        // capitalized
        final List<LoanCharge> capitalizedCharges = new ArrayList<>();
    ...
    public LoanScheduleModel assembleForInterestRecalculation(final LoanApplicationTerms loanApplicationTerms, final Long officeId,
            final List<LoanTransaction> transactions, final Loan loan,
    ...
    public LoanRepaymentScheduleInstallment calculatePrepaymentAmount(final MonetaryCurrency currency, final LocalDate onDate,
            final LoanApplicationTerms loanApplicationTerms, final Loan loan, final Long officeId,
        ...
        final List<WorkingDayExemptionsData> workingDayExemptions = this.workingDayExcumptionsReadPlatformService
                .getWorkingDayExemptionsForEntityType(EntityAccountType.LOAN.getValue());
        final HolidayDetailDTO holidayDetailDTO = new HolidayDetailDTO(isHolidayEnabled, holidays, workingDays, workingDayExemptions);
    ...
    public void assempleVariableScheduleFrom(final Loan loan, final String json) {
        ...
        final List<LoanTermVariations> variations = loan.getLoanTermVariations();
        ...
        if (!variations.isEmpty()) {
            final List<LoanTermVariations> retainVariations = adjustExistingVariations(variations, newVariations, adjustDueDateVariations);
        ...
        final Set<LocalDate> dueDates = new TreeSet<>();
        ...
        final Collection<LocalDate> keySet = adjustDueDateVariations.keySet();
        dueDates.addAll(keySet);
        for (final LocalDate date : keySet) {
            final LocalDate removeDate = adjustDueDateVariations.get(date);
            if (removeDate != null) {
                dueDates.remove(removeDate);
            }
        }

        final Set<LocalDate> actualDueDates = new TreeSet<>(dueDates);
        final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
        final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors).resource("loan");
        final List<LocalDate> overlappings = new ArrayList<>();
        for (final LoanTermVariations termVariations : variations) {
        ...
        final LoanProductVariableInstallmentConfig installmentConfig = loan.loanProduct().loanProductVariableInstallmentConfig();
        final CalendarInstance loanCalendarInstance = this.calendarInstanceRepository.findCalendarInstaneByEntityId(loan.getId(),
        ...
        for (final LocalDate duedate : dueDates) {
            final int gap = Days.daysBetween(previousDate, duedate).getDays();
    ...
    private List<LoanTermVariations> adjustExistingVariations(final List<LoanTermVariations> variations,
            final List<LoanTermVariations> newVariations, final Map<LocalDate, LocalDate> adjustDueDateVariations) {
        final Map<LocalDate, LoanTermVariations> amountVariations = new HashMap<>();
        final Map<LocalDate, LoanTermVariations> dueDateVariations = new HashMap<>();
        final Map<LocalDate, LoanTermVariations> insertVariations = new HashMap<>();
        for (final LoanTermVariations loanTermVariations : variations) {
        ...
        final List<LoanTermVariations> retainVariations = new ArrayList<>();
        for (final LoanTermVariations loanTermVariations : newVariations) {
            boolean retain = true;
            switch (loanTermVariations.getTermType()) {
                case DUE_DATE:
                    ...
                    if (dueDateVariations.containsKey(loanTermVariations.fetchTermApplicaDate())) {
                        final LoanTermVariations existingVariation = dueDateVariations.get(loanTermVariations.fetchTermApplicaDate());
    ...
    private void extractLoanTermVariations(final Loan loan, final String dateFormat, final Locale locale,
        ...
        for (int i = 1; i <= modificationsArray.size(); i++) {
            ...
            final LocalDate duedateLocalDate = this.fromApiJsonHelper.extractLocalDateNamed(LoanApiConstants.dueDateParamName, arrayElement,
                    dateFormat, locale);
            final Date dueDate = duedateLocalDate.toDate();

            final LocalDate modifiedDuedateLocalDate = this.fromApiJsonHelper
                    .extractLocalDateNamed(LoanApiConstants.modifiedDueDateParamName, arrayElement, dateFormat, locale);
            Date modifiedDuedate = null;
            if (modifiedDuedateLocalDate != null) {
                modifiedDuedate = modifiedDuedateLocalDate.toDate();
            }
            final boolean isSpecificToInstallment = true;
            if (isInsertInstallment) {
                final LoanTermVariations data = new LoanTermVariations(LoanTermVariationType.INSERT_INSTALLMENT.getValue(), dueDate,
                        decimalValue, modifiedDuedate, isSpecificToInstallment, loan);
                loanTermVariations.add(data);
            } else if (isDeleteInstallment) {
                final LoanTermVariations data = new LoanTermVariations(LoanTermVariationType.DELETE_INSTALLMENT.getValue(), dueDate,
                        decimalValue, modifiedDuedate, isSpecificToInstallment, loan);
                loanTermVariations.add(data);
            } else {
                if (modifiedDuedate != null) {
                    final BigDecimal amountData = null;
                    final LoanTermVariations data = new LoanTermVariations(LoanTermVariationType.DUE_DATE.getValue(), dueDate, amountData,
                ...
                if (decimalValue != null) {
                    ...
                    final LoanTermVariations data = new LoanTermVariations(decimalValueVariationType.getValue(), modifiedDuedate,
                            decimalValue, date, isSpecificToInstallment, loan);
                    loanTermVariations.add(data);
                }
            }
