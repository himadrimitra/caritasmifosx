...
public class SavingsProductAssembler {
    ...
    private List<FloatingInterestRateChart> constructFloatingInterestRateChart(final String json) {
        final List<FloatingInterestRateChart> floatingInterestRateCharts = new ArrayList<>();
        ...
        if (element.isJsonObject()) {
            if (topLevelJsonElement.has(floatingInterestRateChartParamName)) {
                ...
                for (int i = 0; i < array.size(); i++) {
                    ...
                    final FloatingInterestRateChart floatingInterestRateChart = createFloatingInterestRateChart(interestrateChartElement,
                            locale, dateFormat);
    ...
    private FloatingInterestRateChart createFloatingInterestRateChart(final JsonObject interestrateChartElement, final Locale locale,
        ...
        final BigDecimal interestRate = this.fromApiJsonHelper.extractBigDecimalNamed(interestRateParamName, interestrateChartElement,
                locale);
        return new FloatingInterestRateChart(effectiveFromDate, interestRate);

    }

    public List<FloatingInterestRateChart> updateFloatingInterestRateChart(final SavingsProduct product, final Map<String, Object> changes,
            final String json) {
        final List<FloatingInterestRateChart> existingFloatingInterestRateCharts = product.getFloatingInterestRateChart();
        final Map<Long, FloatingInterestRateChart> existigFloatingInterestRateChartMap = new HashMap<>();
        if (existingFloatingInterestRateCharts.size() > 0) {
            for (final FloatingInterestRateChart floatingInterestRateChart : existingFloatingInterestRateCharts) {
        ...
        if (element.isJsonObject()) {
            if (topLevelJsonElement.has(floatingInterestRateChartParamName)) {
                ...
                for (int i = 0; i < array.size(); i++) {
                    ...
                    if (id == null) {
                        final FloatingInterestRateChart floatingInterestRateChart = createFloatingInterestRateChart(
                                interestrateChartElement, locale, dateFormat);
                        ...
                        final FloatingInterestRateChart floatingInterestRateChart = existigFloatingInterestRateChartMap.get(id);
    ...
    private void validateFloatingInterestRateChart(final List<FloatingInterestRateChart> floatingInterestRateCharts) {
        final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
        final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors)
                .resource(INTERESTRATE_CHART_RESOURCE_NAME);
        final List<LocalDate> effectiveFromDateList = new ArrayList<>();
        for (final FloatingInterestRateChart chart : floatingInterestRateCharts) {
    ...
    private void validateForDelete(final FloatingInterestRateChart floatingInterestRateChart) {
        final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
        final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors)
                .resource(INTERESTRATE_CHART_RESOURCE_NAME);
